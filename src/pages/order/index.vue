<template>
  <div class="order">
    <div class="order-tab fz1 mb-10 bg-f">
      <div class="order-tab-item " v-for="item in tab" :key="item.index" @click="getList(item.type)" :class="[type==item.type?'active':'']">{{item.name}}</div>
    </div>
...
export default {
  ...
  methods: {
   ...
   async init() {
      ...
      if(JSON.stringify(res.data)=='{}'||(res.data.OrderListShow&&res.data.OrderListShow.length==0)) {
